...
// adminOrPermission: admin gets through always; worker needs the specified permission level
// usage: adminOrPermission('projects', 'read')
function adminOrPermission(menuKey, level = 'read') {
  return async (req, res, next) => {
...
router.post('/departments', auth, adminOrPermission('departments','write'), auditLog('CREATE','Department'), async (req, res) => {
  const db = await getPool();
...
router.put('/departments/:id', auth, adminOrPermission('departments','edit'), auditLog('UPDATE','Department'), async (req, res) => {
  const db = await getPool();
...
router.post('/workers', auth, adminOrPermission('workers','write'), auditLog('CREATE','Worker'), async (req, res) => {
  const db = await getPool();
...
router.put('/workers/:id', auth, adminOrPermission('workers','edit'), auditLog('UPDATE','Worker'), async (req, res) => {
  const db = await getPool();
...
router.delete('/workers/:id', auth, adminOrPermission('workers','delete'), auditLog('DELETE','Worker'), async (req, res) => {
  const db = await getPool();
  try {
...
router.patch('/workers/:id/rate', auth, adminOrPermission('workers','edit'), async (req, res) => {
  const db = await getPool();
...
router.put('/projects/:id', auth, adminOrPermission('projects','edit'), auditLog('UPDATE','Project'), async (req, res) => {
  const db = await getPool();
...
router.post('/projects/:id/mo-references', auth, adminOrPermission('projects','write'), moReferenceUpload.array('images', 10), async (req, res) => {
  const db = await getPool();
...
router.delete('/projects/:projectId/mo-references/:fileId', auth, adminOrPermission('projects','delete'), async (req, res) => {
  const db = await getPool();
...
router.get('/reports/mo-references', auth, adminOrPermission('projects','read'), async (req, res) => {
  const db = await getPool();
  try {
    const [rows] = await db.query(`
      SELECT
...
router.delete('/projects/:id', auth, adminOrPermission('projects','delete'), auditLog('DELETE','Project'), async (req, res) => {
  const db = await getPool();
  try {
...
// Get images grouped by item → stage for a project (project-wise, no cross-project mixing)
router.get('/projects/:id/images-by-stage', auth, async (req, res) => {
  ...
  try {
    ...
    // Get chains for this project (item-aware)
    const [chains] = await db.query(`
      SELECT pc.project_item_id, pc.stage_order, pc.department_id,
             d.name as dept_name, d.color as dept_color
      FROM production_chains pc
      JOIN departments d ON d.id=pc.department_id
      WHERE pc.project_id=?
      ORDER BY pc.project_item_id, pc.stage_order`, [projectId]);

    // Get all images for this project ONLY (strict WHERE ta.project_id=?)
    const [allImages] = await db.query(`
      SELECT ti.id, ti.image_path, ti.caption, ti.created_at,
             u.name as uploaded_by_name, ta.task_title,
             ta.department_id, ta.stage_order, ta.project_item_id
      FROM task_images ti
      JOIN task_assignments ta ON ta.id=ti.task_id
      JOIN users u ON u.id=ti.uploaded_by
      WHERE ta.project_id=?
      ORDER BY ta.project_item_id, ta.stage_order, ti.created_at DESC`, [projectId]);

    // Group images: key = itemId_stageOrder_deptId
    const imageMap = {};
    ...
    // Build result: per item → per stage
    const result = items.map(item => {
      const itemChains = chains.filter(c => c.project_item_id === item.id);
      const stages = itemChains.map(chain => ({
        stage_order: chain.stage_order,
        dept_name: chain.dept_name,
        dept_color: chain.dept_color,
        images: imageMap[`${item.id}_${chain.stage_order}_${chain.department_id}`] || []
      }));
      return {
        item_id: item.id,
    ...
    }).filter(item => item.stages.some(s => s.images.length > 0)); // only items with images
...
router.post('/projects/:projectId/items/:itemId/images', auth, adminOrPermission('projects','write'), imgUpload.array('images', 10), async (req, res) => {
  const db = await getPool();
  try {
    const files = req.files || [];
...
router.delete('/projects/:projectId/items/:itemId/images/:imageId', auth, adminOrPermission('projects','delete'), async (req, res) => {
  const db = await getPool();
  try {
...
router.post('/projects/:id/chain', auth, adminOrPermission('projects','edit'), async (req, res) => {
  try {
    const db = await getPool();
...
// ── TASKS ─────────────────────────────────────────────────────────────────────
router.get('/tasks/all', auth, adminOrPermission('tasks','read'), async (req, res) => {
  const db = await getPool();
...
router.post('/tasks', auth, adminOrPermission('tasks','write'), auditLog('CREATE','Task'), async (req, res) => {
  const db = await getPool();
...
router.put('/tasks/:id', auth, adminOrPermission('tasks','edit'), auditLog('UPDATE','Task'), async (req, res) => {
  const db = await getPool();
...
router.delete('/tasks/:id', auth, adminOrPermission('tasks','delete'), auditLog('DELETE','Task'), async (req, res) => {
  const db = await getPool();
  try {
...
router.post('/tasks/:id/transfer', auth, adminOrPermission('tasks','edit'), async (req, res) => {
  const db = await getPool();
...
router.delete('/departments/:id/gallery/:fileId', auth, adminOrPermission('departments','delete'), async (req, res) => {
  const db = await getPool();
...
//
// An item is "pending in dispatch" only when it has a DISPATCH task_assignment
// AND every non-dispatch stage of that item is completed (= it has reached the
// Dispatch Department). All quantities are driven by that dispatch task — the
// same record the Stage Tracker / worker UI write to:
//   qty_to_dispatch = dispatch task.quantity_assigned   (qty that must dispatch)
//   dispatched      = SUM(daily_progress.qty_done) on the dispatch dept
//                     (kept in lock-step with quantity_completed by the
//                      progress-update endpoint).
//
// Per item, against the selected day/range [from, to]:
//   In      = qty that REACHED dispatch within [from, to]
//             (its last pre-dispatch stage was approved in the period).
//   Opening = pending dispatch qty carried in from before `from`
//             = qty_to_dispatch − dispatched-before-`from`  (reached before `from`).
//   Out     = qty dispatched within [from, to].
//   Closing = Opening + In − Out  (computed per PROJECT, clamped at 0 — identical
//             to the frontend formula, so report total == dashboard pending total).
//
// Rule 6 / "only currently pending": a project is returned ONLY when its Closing
...
//   5:from 6:to   → dispatched-within-period
    ...
    FROM (
      ...
      FROM (
        ...
        SELECT
          ...
          COALESCE(
            (SELECT MAX(COALESCE(pre.completed_date, DATE(pre.updated_at)))
               FROM task_assignments pre
               JOIN departments pd ON pd.id = pre.department_id
              WHERE pre.project_item_id = dt.project_item_id
                AND LOWER(pd.name) NOT LIKE '%dispatch%'),
            DATE(MIN(dt.created_at))
          ) AS reached_at
        FROM task_assignments dt
        JOIN departments dd0 ON dd0.id = dt.department_id
        WHERE LOWER(dd0.name) LIKE '%dispatch%'
          /* every non-dispatch stage of this item is completed = it reached dispatch */
          AND NOT EXISTS (
            SELECT 1 FROM task_assignments tn JOIN departments dn ON dn.id = tn.department_id
             WHERE tn.project_item_id = dt.project_item_id
      ...
      ) dir ON dir.project_item_id = disp.project_item_id
...
router.get('/reports/dashboard', auth, async (req, res) => {
  ...
  try {
    ...
    const [[projStats]] = await db.query(`
      SELECT COUNT(*) as total,
        SUM(status='active')             as active,
        SUM(status='completed')          as completed,
        SUM(status='on_hold')            as on_hold,
        SUM(status='inactive')           as inactive,
        SUM(status NOT IN ('deleted','cancelled','completed')
            AND deadline IS NOT NULL AND deadline < CURDATE()) as overdue
      FROM projects`);
    projStats.in_progress = 0;

    const [[workerStats]] = await db.query(
      `SELECT COUNT(*) as total FROM users WHERE role='worker' AND is_active=1`);

    // Extra stat card counts — no date filter
    let extraStats = { items_running: 0, pending_approvals: 0, dispatch_worker_qty: 0, dispatch_challan_qty: 0, dispatch_pending_qty: 0 };
    try {
      const [[es]] = await db.query(`
        SELECT
          ...
          (SELECT COALESCE(SUM(dp.qty_done), 0)
            FROM daily_progress dp
            JOIN departments d ON d.id = dp.department_id
            WHERE LOWER(d.name) LIKE '%dispatch%'
              AND dp.work_date = CURDATE()
          ) AS dispatch_worker_qty,
          (SELECT COALESCE(SUM(sci.quantity), 0)
            FROM sale_challan_items sci
            JOIN sale_challans sc ON sc.id = sci.challan_id
            WHERE sc.challan_date = CURDATE()
              AND sc.status != 'cancelled'
          ) AS dispatch_challan_qty`);
      extraStats = es;
    } catch(e) {
      console.warn('extraStats partial error:', e.message);
    }
      ...
      SELECT
        ...
        (SELECT COUNT(*) FROM task_assignments ta2
          WHERE ta2.project_id = p.id
            AND ta2.status NOT IN ('completed','cancelled')
            AND ta2.due_date IS NOT NULL AND ta2.due_date < CURDATE()
        ) AS overdue_tasks,
        (SELECT d2.name
          FROM task_assignments ta3
          JOIN departments d2 ON d2.id = ta3.department_id
          WHERE ta3.project_id = p.id AND ta3.status = 'in_progress'
          ORDER BY COALESCE(ta3.stage_order, d2.stage_order, 99) ASC
          LIMIT 1
        ) AS current_stage
      FROM projects p
      WHERE p.status NOT IN ('deleted','cancelled','completed')
    ...
    const [projectHealthData] = await db.query(`
      SELECT p.id, p.name, p.status, p.deadline,
        ...
        (SELECT d2.name
          ...
          WHERE ta2.project_id = p.id AND ta2.status = 'in_progress'
    ...
    const [deptWorkload] = await db.query(`
      SELECT d.id, d.name, d.color, d.stage_order,
        ...
        COALESCE(SUM(CASE WHEN ta.status='in_progress' THEN 1 ELSE 0 END), 0) AS running,
        COALESCE(SUM(CASE WHEN ta.status='pending'     THEN 1 ELSE 0 END), 0) AS pending,
        COALESCE(SUM(CASE WHEN ta.status='completed'
          AND DATE(ta.updated_at) BETWEEN ? AND ? THEN 1 ELSE 0 END), 0) AS completed,
        COALESCE(SUM(CASE WHEN ta.status='in_progress'
          AND ta.updated_at < DATE_SUB(NOW(), INTERVAL 24 HOUR) THEN 1 ELSE 0 END), 0) AS delayed_24h,
    ...
    // ── 5. LIVE ITEM TRACKING (no date filter — always current) ─────────────
    const [liveItemTracking] = await db.query(`
      SELECT
        ta.id, ta.task_title,
    ...
    // ── 6. STAGE FLOW SUMMARY ────────────────────────────────────────────────
    // total_working = tasks in_progress/pending BEFORE from date (opening balance)
    // today_in      = tasks created/assigned in [from, to]
    // today_out     = tasks completed in [from, to]
    // pending_count = currently pending tasks
    const [stageFlowSummary] = await db.query(`
      SELECT d.id, d.name, d.color, d.stage_order,

        /* Opening = active tasks BEFORE the from date */
        COALESCE(SUM(
        ...
        /* In = tasks created/assigned in selected range */
        COALESCE(SUM(
          CASE WHEN DATE(ta.created_at) BETWEEN ? AND ?
            AND ta.status != 'cancelled'
          THEN 1 ELSE 0 END
        ), 0) AS today_in,

        /* Out = tasks completed in selected range */
        COALESCE(SUM(
    ...
    // ── 8. WORKER DAILY REPORT ───────────────────────────────────────────────
    // Opening = backlog BEFORE from date
    // Today In  = tasks/qty assigned in [from, to]
    // Today Out = daily_progress qty_done in [from, to]
    ...
    try {
      [workerDailyReport] = await db.query(`
        SELECT
          ...
          /* Opening = active backlog BEFORE from date */
          COALESCE(SUM(
            CASE WHEN ta.status IN ('in_progress','pending')
              AND DATE(ta.created_at) < ?
          ...
          /* Today In = new tasks assigned in selected range */
          COALESCE(SUM(
            CASE WHEN DATE(ta.created_at) BETWEEN ? AND ?
              AND ta.status != 'cancelled'
    ...
    // ── 9. DISPATCH PENDING REPORT — PROJECT-LEVEL SUMMARY ───────────────────
    // One row per project = totals of all its items (Opening / In / Out / Closing).
    // Computed ONCE for ALL pending projects so the dashboard "pending dispatch"
    // total and this report can never disagree: the card = SUM(closing) over the
    // exact same rows, and the inline table just shows the top slice of them.
    ...
    const [workerPerformance] = await db.query(`
      ...
      WHERE u.role='worker' AND u.is_active=1
      GROUP BY u.id ORDER BY completed DESC LIMIT 8`);
...
router.get('/reports/workers', auth, adminOrPermission('reports','read'), async (req, res) => {
  const db = await getPool();
  try {
    const [rows] = await db.query(`
...
router.get('/reports/departments', auth, adminOrPermission('reports','read'), async (req, res) => {
  const db = await getPool();
  try {
    const [rows] = await db.query(`
      SELECT d.id, d.name, d.color,
...
router.get('/reports/transfers', auth, adminOrPermission('reports','read'), async (req, res) => {
  const db = await getPool();
  const [rows] = await db.query(`
...
router.get('/reports/material-requests', auth, adminOrPermission('reports','read'), async (req, res) => {
  const db = await getPool();
  try {
...
// Time report
router.get('/reports/time', auth, adminOrPermission('reports','read'), async (req, res) => {
  const db = await getPool();
...
router.get('/reports/daily-progress', auth, adminOrPermission('daily_progress','read'), async (req, res) => {
  const db = await getPool();
...
router.delete('/materials/:id', auth, adminOrPermission('materials','delete'), auditLog('DELETE','Material'), async (req, res) => {
  const db = await getPool();
  try {
...
router.delete('/production-output/:id', auth, adminOrPermission('materials','delete'), async (req, res) => {
  const db = await getPool();
  try {
...
router.get('/reports/materials', auth, adminOrPermission('materials','read'), async (req, res) => {
  const db = await getPool();
...
router.patch('/queries/:id/respond', auth, adminOrPermission('queries','edit'), auditLog('UPDATE','Query'), async (req, res) => {
  const db = await getPool();
...
router.patch('/queries/:id/status', auth, adminOrPermission('queries','edit'), auditLog('UPDATE','Query'), async (req, res) => {
  const db = await getPool();
...
router.put('/settings', auth, adminOrPermission('settings','edit'), auditLog('UPDATE','Settings'), async (req, res) => {
  const db = await getPool();
  try {
...
router.delete('/departments/:id/force', auth, adminOrPermission('departments','delete'), async (req, res) => {
  const db = await getPool();
  try {
...
// ── DEPARTMENT REORDER ───────────────────────────────────────────
router.post('/departments/reorder', auth, adminOrPermission('departments','edit'), async (req, res) => {
  const db = await getPool();
  try {
...
router.post('/workers/bulk-departments', auth, adminOrPermission('workers','edit'), async (req, res) => {
  const db = await getPool();
...
router.get('/packing/boxes', auth, async (req, res) => {
  ...
  try {
    ...
    // Get items for ALL boxes in ONE query (fixes the load issue).
    // Previously this fired one SELECT per box in a sequential await loop
    // (N+1), so as the live data grew the page kept getting slower and
    // eventually timed out -> "Load nahi hua" / "Failed to load".
    const boxIds = boxes.map(b => b.id);
    if (boxIds.length > 0) {
      const [allItems] = await db.query(
        'SELECT * FROM packing_box_items WHERE box_id IN (?) ORDER BY box_id, id',
        [boxIds]
      );
      const itemsByBox = new Map();
      for (const it of allItems) {
        if (!itemsByBox.has(it.box_id)) itemsByBox.set(it.box_id, []);
        itemsByBox.get(it.box_id).push(it);
      }
      boxes.forEach(box => { box.items = itemsByBox.get(box.id) || []; });
    }
    res.json(boxes);
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
router.post('/packing-parts', auth, adminOrPermission('packing','write'), async (req, res) => {
  const db = await getPool();
  try {
...
router.delete('/packing-parts/:id', auth, adminOrPermission('packing','delete'), async (req, res) => {
  const db = await getPool();
...
// GET all challans
router.get('/sale-challans', auth, adminOrPermission('sale_challans','read'), async (req, res) => {
  const db = await getPool();
  try {
    const { project_id, status, search } = req.query;
    let sql = `
...
router.get('/sale-challans/:id', auth, adminOrPermission('sale_challans','read'), async (req, res) => {
  const db = await getPool();
  try {
...
// PUT update challan
router.put('/sale-challans/:id', auth, adminOrPermission('sale_challans','edit'), auditLog('UPDATE','SaleChallan'), async (req, res) => {
  const db = await getPool();
  try {
    const { client_name, client_phone, client_address, client_gstin, challan_type,
      project_id, challan_date, delivery_date, status, notes,
...
// PATCH status only
router.patch('/sale-challans/:id/status', auth, adminOrPermission('sale_challans','edit'), auditLog('UPDATE','SaleChallan'), async (req, res) => {
  const db = await getPool();
  try {
    const { status } = req.body;
...
// DELETE challan
router.delete('/sale-challans/:id', auth, adminOrPermission('sale_challans','delete'), auditLog('DELETE','SaleChallan'), async (req, res) => {
  const db = await getPool();
  try {
...
router.post('/settings/test-email', auth, adminOrPermission('settings','edit'), async (req, res) => {
  try {
    const db = await getPool();
...
router.get('/admin/client-purchase-orders', auth, adminOrPermission('client_pos','read'), async (req, res) => {
  const db = await getPool();
  try {
    const [pos] = await db.query(
...
router.get('/admin/client-purchase-orders/:id', auth, adminOrPermission('client_pos','read'), async (req, res) => {
  const db = await getPool();
  try {
    const [[po]] = await db.query(
      `SELECT cpo.*,
...
router.get('/admin/client-purchase-orders/:id/fulfillment', auth, adminOrPermission('client_pos','read'), async (req, res) => {
  const db = await getPool();
  try {
    const [[po]] = await db.query(
...
router.patch('/admin/client-purchase-orders/:id/status', auth, adminOrPermission('client_pos','edit'), async (req, res) => {
  const db = await getPool();
  try {
    const { status } = req.body;
...
router.patch('/admin/client-purchase-orders/:id/project-name', auth, adminOrPermission('client_pos','edit'), async (req, res) => {
  const db = await getPool();
  try {
...
router.post('/admin/client-purchase-orders/:id/create-project', auth, adminOrPermission('client_pos','write'), async (req, res) => {
  const db = await getPool();
  try {
    const [[po]] = await db.query(
...
router.post('/admin/client-purchase-orders/:id/create-items', auth, adminOrPermission('client_pos','write'), async (req, res) => {
  const db = await getPool();
  try {
...
router.post('/sale-challans/:id/upload-pdf', auth, adminOrPermission('sale_challans','edit'), clientPoUpload.single('pdf'), async (req, res) => {
  const db = await getPool();
  try {
...
router.patch('/sale-challans/:id/send-to-client', auth, adminOrPermission('sale_challans','edit'), async (req, res) => {
  const db = await getPool();
  try {
